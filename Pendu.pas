...
procedure PropoMot (var T1:Tab1;var Lettre:char;Var Mot:string;var compteur : integer; var Vie : integer; var nouveaumot : string ); 
var 
	result : integer;
	
	
begin
	If Compteur = round(TailleDuMot(Mot)DIV 3) then
		begin 
			writeln('voulez vous tenter d''ecrire un mot ?  1 oui 2 non');
			readln(result);
			if (result = 1) then 
			begin
			writeln ('taper le mot '); 
			readln (nouveaumot);
				if nouveaumot <> (mot) then 
				Vie := Vie -1;
			end;
		end;	
...
BEGIN
	...
	Until ((Vie=0) OR (Compteur=TailleDuMot(Mot)));
		PropoMot( T1, Lettre, Mot, compteur ,  Vie ,nouveaumot);
		if nouveaumot = mot then 
		writeln ('vous avez trouve le mot'); 
	readln;
	clrscr;
	If Vie=0 then
		begin
